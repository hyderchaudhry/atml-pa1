...
4. Call `generate_conflicts()`, visually inspect the generated images, and fill `accepted` and `rejection_reason` in `results/cue_conflict_manifest.json`. Decide using the stated visual rule only. Then call `evaluate_cue_conflicts()` and run the representation section. At least 200 accepted images are required. Recalling `generate_conflicts()` preserves an existing reviewed manifest.
