...
4. Call `generate_conflicts()`, visually inspect the generated images, and fill `accepted` and `rejection_reason` in `results/cue_conflict_manifest.json`. Decide using the stated visual rule only. Then call `evaluate_cue_conflicts()` and run the representation section. At least 200 accepted images are required.
...
The notebook writes small JSON artifacts to `results/`: `test_subset_manifest.json`, `condition_metrics.json`, `translation_metrics.json`, `cue_conflict_manifest.json`, `cue_conflict_acceptance_counts.json`, `cue_conflict_metrics.json`, and `representation_stability.json`. Plots appear inline when `CONFIG["plot"]` is true. Generated conflict images live under `results/cue_conflicts/`; do not commit large generated images.
