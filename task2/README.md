...
Install [`requirements.txt`](../requirements.txt), then place PACS in `../data/PACS/<domain>/<class>/` using the notebook's lowercase domain names: `photo`, `art_painting`, `cartoon`, and `sketch`. Check the first cell's `CONFIG` before running. Its defaults specify seed `6304`, 30 epochs, patience 5, AdamW learning rate `1e-4`, weight decay `1e-4`, eight examples from each source per update, and 24 unlabeled target examples for adaptation methods. `plot` and `print_metrics` control inline output.
...
Source-only optimizes source cross-entropy and does not iterate the target loader. DAN adds a three-bandwidth RBF MMD penalty between pooled source and target features. DANN applies a scheduled gradient-reversal signal from a binary domain discriminator. CDAN feeds the discriminator the outer product of features and soft class probabilities. Checkpoints are chosen by the mean of the three source-validation macro-F1 values, without consulting target labels.

`results/source_only.pt` is the shared ERM checkpoint required by Task 3. The final section writes `results/final_metrics.csv` with source/target aggregate metrics and source-vs-target domain separability, and `results/mmd_strength_study.csv` for the fixed `{0.1, 1, 10}` DAN study. The notebook can show a classification-loss curve inline. The current notebook does not yet save the manual's per-class target changes or confusion cases as a result table; these must be added before claiming complete Task 2 evidence.
