...
Source diagnostics include per-domain accuracy and macro-F1, mean/worst source values, a balanced three-way logistic-regression source-domain separability probe, and a common radius-0.05 sharpness proxy on a fixed validation batch. The final section computes Sketch aggregate metrics and a preconfigured SAM-radius study. Outputs are `results/source_diagnostics.csv`, `results/final_metrics.csv`, `results/sketch_per_class.csv`, `results/sketch_confusions.csv`, `results/sam_strength_study.csv`, and DAN-DG/SAM history CSVs. With `print_metrics=True`, the notebook displays the source, final, per-class, dominant-confusion, study, and final-epoch tables. With `plot=True`, it displays classification and MMD curves inline.
