...
Run [Task 2](../task2/README.md) first so the repository's `../common/splits/pacs_sketch_seed6304.json` and Drive's `/content/drive/MyDrive/ATML_PA1/checkpoints/task2/source_only.pt` exist. Install [`requirements.txt`](../requirements.txt) and use the same Drive PACS directory layout. The notebook mounts Drive in Colab. Review the first cell's `CONFIG` before running: seed `6304`, 30 epochs, patience 5, AdamW `1e-4` learning rate and weight decay, eight images per source per update, DAN-DG weight `1`, SAM radius `0.05`, and the fixed SAM radius study `{0.01, 0.05, 0.1}`.
...
The notebook reads Task 2's saved source split and uses the same ResNet-18 architecture, ImageNet weights, preprocessing, domain-balanced batches, and frozen BatchNorm running statistics. Shorter source loaders cycle to cover the longest source loader each epoch. It loads ERM from `source_only.pt`; it does not retrain that baseline. DAN-DG adds the average three-kernel RBF MMD over the Photo–Art, Photo–Cartoon, and Art–Cartoon feature pairs, using only observed sources. SAM makes a normalized ascent perturbation and a second forward/backward pass before an AdamW update. Checkpoints use mean source-validation macro-F1.

Source diagnostics include per-domain accuracy and macro-F1, mean/worst source values, a balanced three-way logistic-regression source-domain separability probe, and a common radius-0.05 sharpness proxy on a fixed validation batch. The final section computes Sketch aggregate metrics and a preconfigured SAM-radius study. Repository outputs are `results/source_diagnostics.csv`, `results/final_metrics.csv`, `results/sketch_per_class.csv`, `results/sketch_confusions.csv`, `results/sam_strength_study.csv`, and DAN-DG/SAM history CSVs. With `print_metrics=True`, the notebook displays the source, final, per-class, dominant-confusion, study, and final-epoch tables. With `plot=True`, it displays classification and MMD curves inline and saves `results/source_training_losses.png`.
