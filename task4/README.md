...
For each score, the final section sets a threshold at the 95th percentile of known validation unknownness. It evaluates near, far, and combined unknowns, produces a three-panel score-distribution plot, and records accepted near/far failures under the Vanilla MLS threshold. `print_metrics=True` displays the OSR and failure tables; `plot=True` displays the score distributions and failure-image grid inline. The notebook writes `results/cifar10_split.json`, `results/vanilla.pt`, `results/gcsc.pt`, `results/proser.pt`, `results/osr_metrics.csv`, and `results/vanilla_mls_accepted_failures.csv`. The PROSER loss uses the strongest of five dummy logits as the paper's aggregated unknown response; the placeholder score compares that response with the strongest known logit, with its rejection threshold calibrated from known validation examples only.
