...
For each score, the final section sets a threshold at the 95th percentile of known validation unknownness. Vanilla's four scores are evaluated against near, far, and all unknowns; the trained-model comparison currently records near and far groups. The notebook produces a three-panel score-distribution plot and saves accepted near/far failures under the Vanilla MLS threshold. It writes `results/cifar10_split.json`, `results/vanilla.pt`, `results/gcsc.pt`, `results/osr_metrics.csv`, and `results/vanilla_mls_accepted_failures.csv`. The current implementation should be checked against the PROSER paper's exact loss and placeholder detection rule before treating its PROSER numbers as a faithful reproduction.
